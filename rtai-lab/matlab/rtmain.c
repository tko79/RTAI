...
static void *rt_HostInterface(void *args)
{
	...
	switch (Request) {
		...
		while (1) {
			...
			rt_returnx(task, (char *)ssGetModelName(rtaiSynchronoscope), MAX_NAMES_SIZE*sizeof(char));
			rt_receivex(task, &Idx, sizeof(int), &len);
			samplingTime = ssGetSampleTime(rtaiSynchronoscope,0);
			rt_returnx(task, &samplingTime, sizeof(float));
		    }
		}
		}
		...
		rt_receivex(task, &rtParameters, sizeof(char), &len);
		rt_GetParameterInfo(MMI, &rtParameters, i);
		rt_returnx(task, &rtParameters.dataValue, sizeof(double));
	    } 
	    break;			
	    }
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
	c = getopt_long(argc, argv, "uvVsweop:f:n:i:l:t:d:y:c:m:", options, &option_index);
	if (c == -1)
	    break;
	switch (c) {
	    case 'v':
