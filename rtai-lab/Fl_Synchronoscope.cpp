...
void Fl_Synchronoscope::value(float v)
{
	if (v < Minimum_Value) {
		Value = Minimum_Value;
		return;
...
float Fl_Synchronoscope::value()
{
	return Value;
}
void Fl_Synchronoscope::initgl()
{
	glViewport(0, 0, w(), h());
...
void Fl_Synchronoscope::drawgrid()
{
	char tick_label[20];
	float tick_value;
...
Fl_Synchronoscope::Fl_Synchronoscope(int x, int y, int w, int h, const char *title):Fl_Gl_Window(x,y,w,h,title)
{
	Start_Deg = -90;
	End_Deg   = 270;
