...
function Makename=gen_make_unix(name,files,libs,Makename)
  ...
  T=["# generated by builder.sce: Please do not edit this file"
     ...
     "MODEL = "+name
     "OBJS = "+strcat(files+'.o',' ')      
     "OBJSSTAN = "+strcat(strsubst(files,name+'_void_io','rtmain')+'.o',' ')
     ""
     ...
     name+"_standalone: $(OBJSSTAN)"
      ascii(9)+"f77 $(FFLAGS) -o $@  $(OBJSSTAN) $(SCILIBS) $(ULIBRARY) -lpthread"];
  mputl(T,Makename)
endfunction
//     "OBJSSTAN="+strcat(strsubst(files,'_void_io','_standalone')+'.o',' ') 
...
// Modified by Roberto Bucher (roberto.bucher@die.supsi.ch)
...
//Inserted and modified by Roberto Bucher (roberto.bucher@die.supsi.ch)

  nact=size(act,'*');
  ncap=size(cap,'*');

  Code=['/*'+part('-',ones(1,40))+' callback at user params updates */ ';
        'void '+cformatline(rdnom+'_upar_update(int index)',70);
        '{'
        '}'
        ''
	'double get_tsamp()'
